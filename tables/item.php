 ...
 * @since  1.6
 */
class ToolsTableitem extends JTable
{
	/**
	...
	{
		parent::__construct('#__tools', 'id', $db);
		$this->_observers = new JObserverUpdater($this);
		JObserverMapper::attachAllObservers($this);
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'ToolsTableitem', array('typeAlias' => 'com_tools.item'));
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_tools.item.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_tools/access.xml',
				"/access/section[@name='item']/"
			);
			$default_actions = JAccess::getAssetRules('com_tools.item.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function store($updateNulls = false)
	{
		...
		$table = JTable::getInstance('Item', 'ToolsTable');
	...
	protected function _getAssetName()
	{
		...
		return 'com_tools.item.' . (int) $this->$k;
	}
